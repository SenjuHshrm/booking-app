...
export class CheckListingInputComponent implements OnInit, OnDestroy {
  ...
  public updateDetails() {
    ...
    this._sub.add(this._staycation.updateStaycation(this.id, update).subscribe({
      next: (res: any) => {
  ...
  private _getListingDetails(id: string) {
    this._sub.add(this._staycation.getStaycationDetails(id).subscribe({
      next: (res: any) => {
        ...
        this.genImg = res.genImgList.map((img: string) => this._basicUtil.setImgUrl(img))
        this.bedrooms = res.bedroomList.map((img: string) => this._basicUtil.setImgUrl(img))
