...
export class CreateAdminComponent implements OnInit, OnDestroy {
  ...
  public validation: any = {
    firstname: [{ type: 'required', msg: 'First name required' }],
    lastname: [{ type: 'required', msg: 'Last name required' }],
    email: [
    ...
    password: [
      ...
      {
        type: 'minlength',
        msg: 'Password should be eight (8) characters minimum',
      },
    ],
    authpass: [
  ...
  public confirmPassword: ValidatorFn = (
    ...
    let p = ctrl!.get('password')?.value,
      c = ctrl!.get('authpass')?.value;
    if (p !== c) {
