...
export class Step11Component implements OnInit {
  ...
  ngOnInit(): void {
    this.formRegPropS11 = <FormGroup<Step11Form>>(
      this.regPropFormRoot.control.get(this.formGroupName)
    );
    this._getTaxStatic();
    this._getEarnerFee();
  }
  ...
  private _getTaxStatic() {
    this.sub.add(
      this._globalStatic.getStaticByType('service_fee').subscribe({
        next: (res: { data: any }) => {
  ...
  private _getEarnerFee() {
    this.sub.add(
      this._globalStatic.getStaticByType('earning_fee').subscribe({
        next: (res: any) => {
