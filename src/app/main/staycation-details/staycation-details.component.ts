...
export class StaycationDetailsComponent implements OnInit, OnDestroy {
  ...
  public didCheckOut: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.amenitiesData = this._activatedRoute.paramMap.subscribe({
      next: (v: ParamMap) => {
        ...
        this._checkDidCheckOut(<string>v.get('id'));
      },
    });
  ...
  private _checkDidCheckOut(id: string) {
    this._t &&
      this._staycation.didCheckOut(id).subscribe({
        next: (res) => {
          this.didCheckOut = res.didCheckOut;
        },
        error: (err) => {
          console.log(err);
        },
